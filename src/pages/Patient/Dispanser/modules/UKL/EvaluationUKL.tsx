...
import {AmbulanceUKL} from "configs/ukl";
...
import Modal, {BTN_CLOSE, BTN_CUSTOM, BTN_OK} from "../../../../../components/Modal";
import {getUKLVisitByPatient, saveUKLVisitByPatient} from "../../../../../api/patient";
...
const EvaluationUKL = (p: EvaluationUKLProps) => {
    ...
    const [form, setForm] = useState({
        evaluations: AmbulanceUKL.map((v, i) => v.evaluations[0]),
        level: 1,
    ...
    const handleReset = () => {
        setForm(f => ({
            ...f,
            evaluations: AmbulanceUKL.map(v => v.evaluations[0]),
            doctorId: 0
        }))
    }

    const loadUKL = async () => {
        await getUKLVisitByPatient({patientId: p.patient.id, cache: false})
            .then(res => {
                setState(s => ({
                    ...s,
                    ukl: res
    ...
    return <div>
        ...
        <div className="d-flex flex-row flex-wrap justify-content-around" style={{minWidth: 400}}>
            <div style={{width: 500, paddingRight: 25}}>
                <UKLGroupItems title={"Ведение карты амб. больного"}>
                    <UKLItem value={form.evaluations?.[0]} item={AmbulanceUKL[0]} onChange={handleChangeEvaluation(0)}/>
                    <UKLItem value={form.evaluations?.[1]} item={AmbulanceUKL[1]} onChange={handleChangeEvaluation(1)}/>
                </UKLGroupItems>
                <UKLGroupItems title={"Первичный осмотр и сроки оказания помощи"}>
                    <UKLItem value={form.evaluations?.[2]} item={AmbulanceUKL[2]} onChange={handleChangeEvaluation(2)}/>
                    <UKLItem value={form.evaluations?.[3]} item={AmbulanceUKL[3]} onChange={handleChangeEvaluation(3)}/>
                    <UKLItem value={form.evaluations?.[4]} item={AmbulanceUKL[4]} onChange={handleChangeEvaluation(4)}/>
                    <UKLItem value={form.evaluations?.[5]} item={AmbulanceUKL[5]} onChange={handleChangeEvaluation(5)}/>
                    <UKLItem value={form.evaluations?.[6]} item={AmbulanceUKL[6]} onChange={handleChangeEvaluation(6)}/>
                </UKLGroupItems>
            </div>
            <div style={{width: 500}}>
                <UKLGroupItems title={"Установление клинического диагноза "}>
                    <UKLItem value={form.evaluations?.[7]} item={AmbulanceUKL[7]} onChange={handleChangeEvaluation(7)}/>
                    <UKLItem value={form.evaluations?.[8]} item={AmbulanceUKL[8]} onChange={handleChangeEvaluation(8)}/>
                    <UKLItem value={form.evaluations?.[9]} item={AmbulanceUKL[9]} onChange={handleChangeEvaluation(9)}/>
                    <UKLItem value={form.evaluations?.[10]} item={AmbulanceUKL[10]}
                             onChange={handleChangeEvaluation(10)}/>
                    <UKLItem value={form.evaluations?.[11]} item={AmbulanceUKL[11]}
                             onChange={handleChangeEvaluation(11)}/>
                </UKLGroupItems>
                <UKLGroupItems title={"Назначение лекарственных препаратов"}>
                    <UKLItem value={form.evaluations?.[12]} item={AmbulanceUKL[12]}
                             onChange={handleChangeEvaluation(12)}/>
                    <UKLItem value={form.evaluations?.[13]} item={AmbulanceUKL[13]}
                             onChange={handleChangeEvaluation(13)}/>
                </UKLGroupItems>
                <UKLGroupItems>
                    <UKLItem value={form.evaluations?.[14]} item={AmbulanceUKL[14]}
                             onChange={handleChangeEvaluation(14)}/>
                    <UKLItem value={form.evaluations?.[15]} item={AmbulanceUKL[15]}
                             onChange={handleChangeEvaluation(15)}/>
                </UKLGroupItems>
